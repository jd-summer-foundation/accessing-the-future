...
- [data/raw/87520_activity.xlsb](raw/87520_activity.xlsb), the building-activity data cube of ABS 8752.0 Building Activity, Australia (one row per Type of Building × Type of Work × Reference Quarter × Sector of Ownership × State), downloadable from the ABS release page [Building Activity, Australia](https://www.abs.gov.au/statistics/industry/building-and-construction/building-activity-australia/latest-release#data-downloads). Used only by the retrofit cost analysis.
...
## Derived from 87520_activity.xlsb (Building Activity cube)

[data/processed/dwelling_mix.csv](processed/dwelling_mix.csv) is built by `scripts/build_dwelling_mix.py` from the `87520_activity` sheet of the raw cube. It keeps "Dwelling units commenced" for new work ("Type of Work" = New) in NSW and WA for the three structure types that map onto the CIE DRIS cost archetypes — `Houses` (house), `Semi-detached, row or terrace houses, townhouses - Total` (townhouse), and `Apartments - Total including those attached to a house` (apartment) — sums them over the most recent 12 quarters, and expresses each type as a share of the three-type total per state. Only "Total Sectors" rows are kept: the cube also carries Private Sector and Public Sector rows whose sum equals Total Sectors, so keeping all sector rows would double-count every dwelling. The retrofit cost analysis (`scripts/retrofit_cost_analysis.py`; methodology in [docs/cost_analysis.md](../docs/cost_analysis.md)) uses these shares to weight the CIE DRIS per-dwelling-type costs into per-state figures. `make dwelling-mix` rebuilds the CSV from the raw cube.
